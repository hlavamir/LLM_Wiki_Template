...
to the LLM's judgment each time. |
...
| **Explicit page conventions** | Required YAML frontmatter (`title`, `type`, `created`/`modified`, `sources`), slug rules, an `## Open questions` section, citation rules, and a no-manual-line-wrap rule. | Makes every page structurally consistent and diffable, rather than leaving format 
